...
main_form::main_form() :
	...
	m_play_recording_btn(new button()),
	m_save_recording_btn(new button()),
	m_load_recording_btn(new button()),
	m_clear_events_lst(new button()),
	...
	m_executor(new Executor())
{
	...
	m_executor->start();
	m_play_recording_btn->parent(*this)
		.text("Play (click the " + Key::KEYBOARD.at(m_executor->get_stop_keycode()) + " key to stop the execution)")
		.location({ 0, 120 })
		.size({ 170,50 })
		.font(xtd::drawing::font(xtd::drawing::system_fonts::default_font(), 10.0f))
		.click += [this]
	{
		std::shared_ptr<Routine> routine = std::make_shared<Routine>();
		for (auto& str : m_events_lst->items())
		{
			routine->add_event(str.to_string().trim());
		}
		not m_executor->is_running() ? m_executor->start() : void();
		m_executor->execute(routine);
	}; 
	

	m_save_recording_btn->parent(*this)
	...
	m_load_recording_btn->parent(*this)
	...
	m_clear_events_lst->parent(*this)
		.text("Clear")
		.location({ 560, 120 })
		.size({ 150,50 })
		.font(xtd::drawing::font(xtd::drawing::system_fonts::default_font(), 15.0f))
